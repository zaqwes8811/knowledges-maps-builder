...
final public class ImmutableIdxGetters {
  ...
  static void get_confluence_idx() {
    ...
    for (Map.Entry<String, Integer> pair:
      ...
      // Если нашелся ключ
      if (occure) {
        Tools.print(word + ", " + summary_frequency);
  ...
  static public HashMap<String, String> get_rest_idx(String node) {
    String sorted_freq_idx_json = Tools.file2string(
  ...
  static public List<String> get_list_sentences(String node) {
    return Tools.file2list(Joiner.on(AppConstants.PATH_SPLITTER)
      .join(
        ProcessorTargets.getPathToIndex(),
  ...
  static public List<String> get_sorted_idx(String node) {
    Tools.print(node);
    String sorted_idx_json = Tools.file2string(
      Joiner.on(AppConstants.PATH_SPLITTER)
        .join(
          ProcessorTargets.getPathToIndex(),
          AppConstants.COMPRESSED_IDX_FOLDER,
  ...
  static public HashMap<String, HashMap<String, String>> getStaticNotes() {
    String metadata_static_notes_json = Tools.file2string(
      Joiner.on(AppConstants.PATH_SPLITTER)
        .join(
          ProcessorTargets.getPathToIndex(),
  ...
  static public void main(String[] args) {
    ...
    for (String stem: filtered_sorted_idx) {
      Tools.print(Joiner.on(",").join(stem, freq_idx.get(stem)));
    }

    Tools.print(get_sorted_idx(node).size());
    Tools.print(filtered_sorted_idx.size());
